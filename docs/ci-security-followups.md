...
1. Add secret and history scanning. Completed by `JPB-33`.
   ...
   - `.gitleaks.baseline.json` is committed as an empty reviewed baseline; add only reviewed, grandfathered historical findings.
   - `SECURITY.md` documents credential rotation steps for confirmed leaks.
...
2. Enable branch protection in GitHub.
   ...
   - Add `Secret Scan / Gitleaks git history scan` once the first scheduled scan is stable.
   - Keep release validation tied to CI results instead of only local command output.

3. Restore full Xcode project build/test CI. This is the next JPB-32 parity slice after the staleness/cleanup check.
