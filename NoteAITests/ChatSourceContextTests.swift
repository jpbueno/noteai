...
final class ChatSourceContextTests: XCTestCase {
    func testContextLabelsWorkspaceSourcesWithSharedLinks() {
        ...
        let taskID = UUID(uuidString: "33333333-3333-3333-3333-333333333333")!
        ...
        let context = ChatSourceContext.build(
            ...
            tasks: [
                TaskItem(id: taskID, title: "Send benchmark deck", description: "Include the updated latency chart.")
            ],
            t5tReports: [
                T5TReport(
                    id: reportID,
        ...
        XCTAssertTrue(context.contains("[T1] Task: Send benchmark deck (noteai://task/\(taskID.uuidString))"))
    ...
    func testSourceLinkParserReadsSharedLinks() {
        ...
        XCTAssertEqual(ChatSourceLink(urlString: "noteai://task/\(id.uuidString)")?.kind, .task)
        XCTAssertEqual(ChatSourceLink(urlString: "noteai://t5t/\(id.uuidString)")?.kind, .t5t)
        XCTAssertEqual(ChatSourceLink(urlString: "noteai://task/\(id.uuidString)")?.id, id)
