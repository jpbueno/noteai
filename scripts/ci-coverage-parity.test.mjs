...
test("macOS CI documents the current Swift gate and deferred Xcode gate", () => {
  ...
  assert.match(macOSWorkflow, /swift test/);

  assert.match(followups, /Restore full Xcode project build\/test CI/);
  assert.match(followups, /xcodebuild -project NoteAI\.xcodeproj -scheme NoteAI -configuration Debug/);
  assert.match(followups, /CODE_SIGNING_ALLOWED=NO build/);
  assert.match(followups, /CODE_SIGNING_ALLOWED=NO test/);
});
...
test("CI runbook records the parity matrix and required branch checks", () => {
  ...
  assert.match(cicdRunbook, /macOS CI \/ Swift package tests/);
